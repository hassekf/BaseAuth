<div>
    <div class="flex flex-col gap-6">
        <x-auth-header :title="__('Create your account')" :description="__('Fill in the details below to get started')" />
        ...
        <form wire:submit.prevent="submit" class="flex flex-col gap-6">
            ...
            @if (config('dau.features.password_confirm'))
                <x-dau::input
            ...
            <div class="flex items-center justify-end">
                <button type="submit" class="w-full rounded-md bg-zinc-900 px-4 py-2 text-sm font-semibold text-white transition hover:bg-zinc-800 focus:outline-none focus:ring-2 focus:ring-zinc-900/20 dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-100 dark:focus:ring-white/20">
                    {{ __('Create account') }}
                </button>
            </div>
        </form>
        ...
        @if (Route::has('login'))
            <div class="space-x-1 text-sm text-center rtl:space-x-reverse text-zinc-600 dark:text-zinc-400">
                ...
                <a class="font-semibold text-zinc-900 hover:text-zinc-700 dark:text-white dark:hover:text-zinc-200" href="{{ route('login') }}" wire:navigate>
                    {{ __('Log in') }}
                </a>
            </div>
        @endif
